...
**Возможные варианты:** не переносить требование; вынести в `logs/` или `reports/`; описать в skill (apm-gov/apm-dev) как опциональный артефакт с путём по соглашению.

**Решение:** Требование **activity reports** сохраняется. Нужно **явно отразить** в плане миграции и в skills (например apm-gov): куда в OpenCode-проекте писать compact activity reports агентов — путь по соглашению (например `reports/` или `logs/activity/`) и формат. При декомпозиции ролей в pack это описать в контракте агента и в навыке.
