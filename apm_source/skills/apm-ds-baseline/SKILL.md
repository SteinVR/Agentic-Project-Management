...
5. **Pre-run self-review**: perform self-review gate per AGENTS.md — focus on code correctness and contract compliance only. Skip DoD output-artifact checks (reports, metrics, diagnostics) — those deliverables do not exist until after the full run. All code issues must be resolved before any run.
