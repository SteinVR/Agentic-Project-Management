...
Resolve the scenario once at the start.

| Dimension | Ask / infer |
|-----------|-------------|
| Artifacts | Existing task + frozen spec vs create spec/task journal first vs ad-hoc work. Do not assume spec paths unless they were established. |
| Verification | Run quality gate / review loop, or skip it for this slice. |
| Execution | Do the work in the main session or delegate it to a worker subagent. |
