...
The specialist role is defined by the subagent config, not by the prompt text. Subagents self-orient from SPEC files and `memory_bank/`.

## Delegation contract
Every delegation includes:
1. **TASK_ID** -- references `memory_bank/specs/SPEC_{TASK_ID}.md` (frozen spec) and `memory_bank/tasks/{TASK_ID}.md` (working journal).
