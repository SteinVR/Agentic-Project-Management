...
4. **Runtime Logging**: Verify that key pipeline boundaries have logging — module entry/exit, data transformations, metric computations, error paths. Missing runtime logs break the feedback loop. Do not require excessive verbosity — structured one-liners at critical points are sufficient.
