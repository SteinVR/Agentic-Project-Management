...
## Worktree awareness
You may run inside a git worktree where `.venv` and `data/` are symlinks to the main tree. Treat symlinked resources as read-only. Write new artifacts (models, checkpoints, experiment outputs) locally in the worktree. When referencing an existing model from the main tree, use the absolute path provided in the delegation contract.
