...
3. **Code Quality**: Beyond spec compliance — bugs, unsafe shortcuts, reliability risks, logical issues, maintainability issues.

4. **Runtime Logging**: Verify that key pipeline boundaries have logging — module entry/exit, data transformations, metric computations, error paths. Do not require excessive verbosity — structured one-liners at critical points are sufficient.

5. **Rank by Impact**: Sort findings by real severity (P0-P3). Do not pad reviews with low-value observations. If the work is sound, say so clearly — a clean review is a valid outcome.
